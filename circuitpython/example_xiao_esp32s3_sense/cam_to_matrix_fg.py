...
Port of ../example/cam_to_matrix_fg.py for the Seeed Studio XIAO ESP32S3 Sense.
...
Sensor orientation (vflip/hmirror below) is copied from the other boards
and will likely need adjusting once this is tried on real hardware.
...
# GPIO14 (used on the other boards) is the camera's CAM_D4 data line on this
# board, so we use GPIO1 (header pin D0) instead — the camera, I2C, SD, and
# UART pins on this board don't touch it.
MATRIX_PIN = microcontroller.pin.GPIO1
...
# This board doesn't expose the BOOT button under any board.* name either,
# so we address it via microcontroller.pin like on the other boards.
